...
1. First, build the docker images for the temporal server, and for the calculator worker (the same image for the math
   operators and the orchestrator)
`docker build -t calculator-worker:latest .`
`docker build -t temporal-server:latest .\temporal_server\`
2. then, deploy all the workers (a worker for each math activity, and one orchestrator)
`helm install temporal-server temporal_server/helm`
`helm install add-worker calculator/workers/helm/add` (change by the specific worker)